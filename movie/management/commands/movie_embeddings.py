...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if all_movies:
            ...
            embedding_vector = np.frombuffer(bytes(random_movie.emb), dtype=np.float32)
            self.stdout.write(f"\n🎲 Embedding para película al azar: '{random_movie.title}'")
            self.stdout.write(f"   Dimensiones: {embedding_vector.shape}")
